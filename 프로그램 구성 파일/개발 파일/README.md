...
- `생성된 노트`: 프로그램이 만든 PDF, HTML, Markdown, 자막 파일이 저장되는 폴더입니다. 처음 실행할 때 자동으로 만들어집니다.
- `다운로드한 동영상`: 유튜브 또는 릴스 링크로 가져온 원본 동영상이 저장되는 폴더입니다. 처음 실행할 때 자동으로 만들어집니다.
- `프로그램 구성 파일`: 실행에 필요한 라이브러리와 개발 파일이 들어 있는 폴더입니다. 삭제하거나 이름을 바꾸면 실행이 안 될 수 있습니다.

## 주요 기능

- YouTube / 공개 Reels 링크 다운로드
- 링크 영상 원본을 루트의 `다운로드한 동영상` 폴더에 저장
- 로컬 동영상 파일 선택
