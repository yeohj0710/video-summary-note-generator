...
```powershell
cd "C:\dev\clipnote-ai\프로그램 구성 파일\개발 파일"
.\run_app.ps1
```
...
설치 파일 없이 저장소 자체를 배포합니다. 사용자는 GitHub에서 `Code` -> `Download ZIP`으로 저장소를 내려받고, 압축을 푼 뒤 `ClipNoteAI.exe`를 실행하면 됩니다.
